    ...
    def main(self):
        ...
        if self.TYPE_CONTROLEUR == "DUMMY":
            ...
            if self.FAIRE_AUTOTUNING:
                print(">>> Auto-Tuning LQR activé.")
                tuner = lqr_optimizer.LQRAutoTuner(self.state, self.target_state, sim_time=self.sim_time)
                result = tuner.optimize()
                print("Resultats optimisation LQR:", result)
                lqr_controller = LQR(result['Q'], result['R'])
            else:
                print(">>> Gains LQR chargés depuis config.py")
        ...
        elif self.TYPE_CONTROLEUR == "FUZZY":
            ...
            if self.FAIRE_AUTOTUNING:
                ...
                output_centers, input_gains, pos_output_centers, pos_input_gains = (
                    result["output_centers"], result["input_gains"], result["pos_output_centers"], result["pos_input_gains"]
                )
            ...
            raise ValueError(f"TYPE_CONTROLEUR '{self.TYPE_CONTROLEUR}' non reconnu. Utilisez 'DUMMY', 'LQR', 'PID', 'FUZZY' ou 'SAC'")
        ...
        for i in range(steps):
            ...
            if self.TYPE_CONTROLEUR == "DUMMY":
                ...
                target_theta = np.clip(target_theta, -config.FUZZY_TARGET_THETA_MAX, config.FUZZY_TARGET_THETA_MAX)
                u = -fuzzy_angle.compute([np.degrees(state[2] - target_theta), -state[3]])
            elif self.TYPE_CONTROLEUR == "SAC":
                u = sac_controller.compute(state)

            # Saturation du rapport cyclique PWM
